...
describe('Auth0APIClient', function () {
  ...
  describe('init', function () {
    describe('with overrides', function () {
      it('uses telemetry set in the `auth0Client` query param if available', function () {
        var telemetryIn = { name: 'test-sdk', version: '1.0.0', env: { envOverride: true } };
        (0, _testUtils.setURL)('https://auth.myapp.com/authorize?auth0Client=' + btoa(JSON.stringify(telemetryIn)));
        var options = {
          audience: 'foo',
      ...
      it.only('uses default telemetry key when outside the ULP', function () {
      ...
      it.only('uses different telemetry key when inside the ULP', function () {
